    ...
    internal static class ListGenerator
    {
        ...
        static ListGenerator()
        {
            ...
            CustomerList = (from c in XDocument.Load("Customers.xml").Root?.Elements("customer")
                            select new Customer()
                            {
                                CustomerID = (string?)c.Element("id") ?? "UnKnow",
                                CustomerName = (string?)c.Element("name") ?? "No name",
                                Address = (string?)c.Element("address") ?? "no Data",
                                City = (string?)c.Element("city") ?? "no Data",
                                Region = (string?)c.Element("region") ?? "no Data",
                                PostalCode = (string?)c.Element("postalcode") ?? "no Data",
                                Country = (string?)c.Element("country") ?? "no Data",
                                Phone = (string?)c.Element("phone") ?? "no Data",
                                Fax = (string?)c.Element("fax") ?? "no Data",
                                Orders = (from o in c.Elements("orders").Elements("order")
                                          select new Order
                                          {
                                              ...
                                              Total = decimal.TryParse((string?)o.Element("total"), out decimal Total) ? Total : -1,
                                          }).ToArray()
                            }).ToList();
